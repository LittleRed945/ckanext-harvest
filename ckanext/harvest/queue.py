...
def delete_queues(job_id,obj_ids):
    ...
    if backend == 'redis':
        ...
        if deleted_job[0] != '':
            log.debug("Delete {} job in gather queue".format(deleted_job[0]))
            log.debug("Delete {} objects in gather queue".format(len(deleted_job)))
            log.info('Redis gather queue purged')
        count=0
        json_obj_ids=[]
        ...
        if deleted_objs[0] != '':
            for obj_id in deleted_objs:
                log.debug("Delete {} object in fetch queue".format(obj_id))
            log.debug("Delete {} objects in fetch queue".format(len(deleted_objs)))
            log.info('Redis fetch queue purged')
